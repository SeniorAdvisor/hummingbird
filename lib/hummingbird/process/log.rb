  ...
  module Process
    class Log < Base
      ...
      def self.init(klass,name)
        super(klass,name)
        klass.class_eval do
          has_many "hummingbird_#{name.to_s.pluralize}".to_sym, -> {where(["owner_class = ? and owner_type = ?",klass.name,name.to_s])}, :class_name=>"Hummingbird::Point", :foreign_key => "owner_id"
        end
